...
describes what users get. Pick a format with `-o`, or let wsrc choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
wsrc <command> -o table   # aligned columns for reading
wsrc <command> -o jsonl   # one JSON object per line, for piping
wsrc <command> -o json    # a single JSON array
wsrc <command> -o csv     # spreadsheet friendly
wsrc <command> -o tsv     # tab-separated
wsrc <command> -o url     # just the URL column
wsrc <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
wsrc <command> --fields id,title,url
```
...
```bash
wsrc <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
wsrc <command>            # a table, because this is a terminal
wsrc <command> | wc -l    # JSONL, because this is a pipe
```
